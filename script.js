...
function calculateRoute() {
  // Hide the "Click here" button and the more info text
  document.querySelector('.moreinfo').style.display = 'none';
  document.getElementById('more_info_text').style.display = 'none';
...
function selectRoute(response, selectedRouteIndex) {
  routeRenderers.forEach((renderer, routeIndex) => {
    if (routeIndex === selectedRouteIndex) {
      renderer.setOptions({
        polylineOptions: {
          strokeColor: '#0000FF', // Blue for selected route
          strokeOpacity: 0.9,
          strokeWeight: 7,
        },
        map: map,
      ...
      renderer.setOptions({
        polylineOptions: {
          strokeColor: '#cdd8ff', // Light blue for unselected routes
          strokeOpacity: 0.7,
          strokeWeight: 5,
        },
        map: map,
  ...
  // Prepare the URL with query parameters for the next page
  const nextPageUrl = `next.html?distance=${encodeURIComponent(distance)}&duration=${encodeURIComponent(duration)}&destination=${encodeURIComponent(destination)}`;

  // Update the href of the "moreinfo" link
  document.getElementById('moreinfo-link').href = nextPageUrl;

  // Show the "Click here" button and the more info text
  document.querySelector('.moreinfo').style.display = 'inline-block';
  document.getElementById('more_info_text').style.display = 'inline'; // Show the more info text

  // Also display the selected route's info
  displayRouteInfo(route, selectedRouteIndex);

  // Get the midpoint for weather updates
